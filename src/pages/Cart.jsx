        ...
        <div className="max-w-[1200px] mx-auto">
          ...
          <div className="grid grid-cols-[1fr_360px] gap-10 items-start text-left max-lg:grid-cols-1">
            <div className="flex flex-col gap-4">
              {items.map(item => (
                <div key={item.id} className="flex items-center gap-5 p-6 bg-[var(--color-card)] border border-[var(--color-border)] rounded-xl transition-all duration-300 animate-fade-up hover:shadow-[var(--shadow-sm,0_2px_8px_rgba(0,0,0,0.06))]">
                  <span className="w-13 h-13 flex items-center justify-center bg-[var(--color-accent-bg,transparent)] rounded-xl shrink-0 text-2xl">
                    {item.icon}
                  </span>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <button onClick={() => updateQty(item.id, -1)} className="w-9 h-9 rounded-lg border border-[var(--color-border)] bg-[var(--color-card)] text-lg cursor-pointer flex items-center justify-center text-[var(--color-text-h)] transition-all duration-300 hover:bg-[var(--color-accent-bg,transparent)] hover:border-[var(--color-accent)] hover:text-[var(--color-accent)]" aria-label="Decrease quantity">−</button>
                  ...
                  <button onClick={() => removeItem(item.id)} className="bg-none border-none text-[14px] text-[var(--color-text)] cursor-pointer p-2 rounded-lg transition-all duration-300 shrink-0 hover:bg-[rgba(239,68,68,0.1)] hover:text-[#ef4444]" aria-label="Remove item">✕</button>
                </div>
              ))}
            </div>
            ...
            <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-xl p-8 sticky top-100 animate-scale-in">
              ...
              <button className="w-full bg-[var(--color-accent)] text-white text-base font-semibold px-7 py-4 rounded-xl border-none cursor-pointer transition-all duration-300 mt-6 shadow-[0_4px_16px_var(--color-accent-border,transparent)] hover:bg-[var(--color-accent-light)] hover:shadow-[0_8px_24px_var(--color-accent-border,transparent)] hover:-translate-y-0.5 active:scale-96">
                Proceed to Checkout →
              </button>
              <Link to="/order" className="block text-center text-[var(--color-text)] text-[15px] no-underline mt-3 py-2.5 rounded-xl transition-colors duration-300 hover:bg-[var(--color-accent-bg,transparent)] hover:text-[var(--color-accent)]">
                Continue Browsing
              </Link>
            </div>
          </div>
